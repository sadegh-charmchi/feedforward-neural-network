...
class NeuralNetwork(Learner):
    ...
    sigmoid activation function (False).
    ...
    * ``'targets'``: Set of possible targets.
    ...
    def initialize(self,input_size,n_classes):
        ...
        for h in range(n_hidden_layers):         
            ...
            self.grad_as += [np.zeros((self.sizes[h],))]  # ... and gradient
        self.hs += [np.zeros((self.n_classes,))]       # output layer
        self.grad_as += [np.zeros((self.n_classes,))]  # ... and gradient
        ##################################################################
        ...
        # pirnt weight dim info
        print "weight matrices dimensions:"
        for m in range(len(self.weights)):
            print "\t" + str(self.weights[m].shape)
        ...
        for m in range(len(self.weights)):
            ...
            for ind, val in ndenumerate(self.weights[m]):
    ...
    def train(self,trainset):
        ...
        for it in range(self.epoch,self.n_epochs):
            for input,target in trainset:
                ...
                self.update()
        # decreasing_constant (dc) (keep lr fixed for the first few iterations)
        if self.epoch > 10:
            self.lr /= 1 + self.dc
        self.epoch = self.n_epochs
    ...
    def fprop(self,input,target):
        ...
        ## PUT CODE HERE ##
        if self.tanh == True:
            self.hs[0] = np.tanh(input.dot(self.weights[0]) + self.biases[0])
            for it in range(1, len(self.weights)-1):
                self.hs[it] = np.tanh(self.hs[it - 1].dot(self.weights[it]) + self.biases[it])
        else:
    ...
    def training_loss(self,output,target):
        ...
        ## PUT CODE HERE ##
        if(target > self.n_classes - 1):
            print "target class out of range :-)"
            return
...
#         print "loss: " + str(loss)
    ...
    def bprop(self,input,target):
        """
        Backpropagation:
        - fills in the hidden layers and output layer gradients in self.grad_as
        ...
        e_y[target] = 1
        self.grad_as[-1] = -(e_y - self.hs[-1])
        # step 2: backpropagate grads through hidden layers
        for k in range(len(self.sizes), -1, -1):
            # compute grads of hidden layer params
            if k == 0:
                self.grad_weights[k] = np.outer(input, self.grad_as[k])
            else:
                self.grad_weights[k] = np.outer(self.hs[k-1], self.grad_as[k])
            self.grad_biases[k] = self.grad_as[k]
            
            if k > 0: # hidden layer below exists!
                # compute grads of hidden layer below
                grad_wrt_h = self.weights[k].dot(self.grad_as[k])
                # compute grads of hidden layer below (before activation)
                if self.tanh == True:
                    self.grad_as[k-1] = np.multiply(grad_wrt_h, 1 - self.hs[k-1] ** 2)
                else:
                    self.grad_as[k-1] = np.multiply(grad_wrt_h, self.hs[k-1] - self.hs[k-1] ** 2)
